...
target_dir = r'd:\Website\Client\AGGC Company Portfolio\AGGC Website'

pine_svg = '''        <!-- Subtle Starry Night & Grid Watermark -->
...
for root, dirs, files in os.walk(target_dir):
    for f in files:
        if f.endswith('.html'):
            ...
            # 1. Remove "Designed with modern web technologies."
            # Using regex to capture variations in indentation and tags around it.
            pattern_text = r'[ \t]*<p[^>]*>.*Designed with modern web technologies\.</p>\n?'
            if re.search(pattern_text, content):
                content = re.sub(pattern_text, '', content)
                modified = True
            
            # Also catch just the span/text if it's not wrapped exactly in <p>
            pattern_text2 = r'[ \t]*<span[^>]*></span>[ \t]*Designed with modern web technologies\.\n?'
            if re.search(pattern_text2, content):
                content = re.sub(pattern_text2, '', content)
                modified = True
                
            # 2. Inject pine_svg and wave_svg into the first hero section
            if 'pineHorizonGrad' not in content:
                # Find the first <section ...> tag which contains text-center
                match = re.search(r'(<section[^>]*text-center[^>]*>\n?)', content)
                if match:
                    ...
                    # Inject pine_svg immediately after section tag
                    content = content.replace(section_tag, section_tag + pine_svg)
                    modified = True
            
            if 'Ultra-Smooth Pro Multi-Layer Wave Divider' not in content:
                # We need to inject the wave_svg right before the closing </section> of the hero section.
                # Since hero section is the first section, let's find the first </section>
                match = re.search(r'(\n?[ \t]*</section>)', content)
                if match:
                    closing_tag = match.group(1)
                    content = content.replace(closing_tag, wave_svg + closing_tag, 1)
                    modified = True
